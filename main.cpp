...
// make cross platform (mac and linux)
// Benchmarks for Scala / Elixir / Go
...
void Check_Long_Lines(std::vector<int>& lines, std::vector<std::string>& input, int line_length, bool ignore_imports)
{
	int count = 1;
	for (const auto& s : input)
	{
		if (s.length() > 6 && s.substr(0, 6) == "import" && ignore_imports)
		{
			++count;
			continue;
		}
...
int main(void)
{
	...
	std::vector <std::experimental::filesystem::directory_entry> clean_files;
	std::vector <std::experimental::filesystem::directory_entry> unclean_files;
	for (auto& f : std::experimental::filesystem::recursive_directory_iterator(actual_path))
	{
		...
		if (!Print_Offending_Lines("The following lines were longer than " + std::to_string(config.long_line_length) +
		...
		if(!Print_Offending_Lines("The following lines didn't have a return type on the next line: ", line_counters))
			isClean = false;

		Check_For_Spaces_Between_Commas(line_counters, lines);
		no_space_between_commas += line_counters.size();
		if (!Print_Offending_Lines("The following lines didn't have a space between comma separated values: ", line_counters))
			isClean = false;
		...
		if (!Print_Offending_Lines("The following lines didn't have a space between if and the parentheses: ", line_counters))
			isClean = false;
		...
		if (isClean)
		{
			clean_files.push_back(f);
		}
		else
		{
			unclean_files.push_back(f);
		}
	}
	...
	std::cout << filecount << " files processed in " << std::chrono::duration_cast<std::chrono::microseconds>(end - begin).count() / 1000 << " milliseconds" << std::endl;
	...
	{
		document << HTML::Text(u.path().filename().string()).addAttribute("style", "color:green;");
	}
	...
	std::string html_path = curr_path + "\\report.html";
